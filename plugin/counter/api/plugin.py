...
from core.plugin_system import ApplicationPlugin, ApplicationPluginManifest
from plugin.counter.models import CounterLog


class CounterApplicationPlugin(ApplicationPlugin):
    """
    ...
    def manifest(self):
        return ApplicationPluginManifest(
            plugin_id="counter",
            display_name="Counter example",
            version="1.0.0",
            description="Example application plugin with routes and a Django model.",
            route_namespace="counter",
            model_packages=("plugin.counter.models",),
            automation_ids=("counter.current-value",),
        )
    ...
    def automation_hooks(self):
        return {"counter.current-value": self._get_current_value}
    ...
    def get_counter(self, request):
        ...
        try:
            ...
            return JsonResponse({"value": current_value, "total_actions": total_actions})
    ...
    def increment(self, request):
        ...
        try:
            ...
            CounterLog.objects.create(action="INCREMENT", value=new_value, comment=comment)

            return JsonResponse(
                {"action": "INCREMENT", "previous_value": current_value, "new_value": new_value}
            )
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=500)
    ...
    def decrement(self, request):
        ...
        try:
            ...
            CounterLog.objects.create(action="DECREMENT", value=new_value, comment=comment)

            return JsonResponse(
                {"action": "DECREMENT", "previous_value": current_value, "new_value": new_value}
            )
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=500)
    ...
    def reset(self, request):
        ...
        try:
            ...
            CounterLog.objects.create(action="RESET", value=0, comment=comment)

            return JsonResponse(
                {"action": "RESET", "previous_value": current_value, "new_value": 0}
            )
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=500)
    ...
    def clear_history(self, request):
        ...
        try:
            ...
            return JsonResponse(
                {"message": f"Cleared {count} history entries", "deleted_count": count}
            )
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=500)


# Temporary import compatibility for code that referenced the old class name.
CounterOCPlugin = CounterApplicationPlugin
