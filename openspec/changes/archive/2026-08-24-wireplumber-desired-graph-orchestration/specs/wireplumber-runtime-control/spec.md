...
The system MAY run PipeWire's PulseAudio compatibility server for processors that have not yet migrated, but SHALL still observe and control their streams through WirePlumber and SHALL NOT treat compatibility as a separate backend.

#### Scenario: Decoder uses PulseAudio protocol
- **WHEN** the current decoder opens capture and playback streams through the PipeWire Pulse server
- **THEN** those streams appear in the same PipeWire runtime inventory and participate in the resolved plan
